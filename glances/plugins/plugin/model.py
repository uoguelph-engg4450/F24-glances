        ...
        if isinstance(self.get_raw(), list) and self.get_raw() is not None and self.get_key() is not None:
            ...
            for i in self.get_raw():
                ...
                for field in listkeys(i):
                    ...
                    if not self.hide_zero:
                        ...
                        if field in self.hide_zero_fields and i[field] >= 0: #change the condition to >= 0 so that we don't show blank data.
                            value['hidden'] = False
                    else:
            ...
            for field in listkeys(self.get_raw()):
                ...
                if not self.hide_zero:
                    ...
                    if field in self.hide_zero_fields and self.get_raw()[field] >= 0: #change the condition to >= 0 so that we don't show blank data.
                        value['hidden'] = False
                else:
                    value['hidden'] = field in self.hide_zero_fields
                ret[field] = value
